// File: backend/controllers/loeController.js
...
// Ensure all models are loaded for population
require('../models/Advisor');
require('../models/Client');
require('../models/ClientInvitation');
...
exports.getLOEByToken = async (req, res) => {
  try {
    ...
    // Handle client data - it might be populated or it might be an invitation ID
    let clientData = null;
    
    // First try to get from Client collection
    if (loe.clientId) {
      try {
        const Client = require('../models/Client');
        const client = await Client.findById(loe.clientId);
        ...
        if (client) {
          ...
          logger.info('✅ [LOE] Client data retrieved from Client collection', {
            loeId: loe._id,
            clientName: clientData.name,
            clientEmail: clientData.email
          });
        } else {
          // Client not found, try ClientInvitation
          logger.info('📄 [LOE] Client not found in Client collection, trying ClientInvitation', {
            loeId: loe._id,
          ...
          if (invitation) {
            clientData = {
              name: `${invitation.clientFirstName} ${invitation.clientLastName}`,
              email: invitation.clientEmail
            };
            
            logger.info('✅ [LOE] Client data retrieved from ClientInvitation', {
              loeId: loe._id,
              clientName: clientData.name,
              clientEmail: clientData.email
            });
          } else {
            logger.error('❌ [LOE] Client not found in either Client or ClientInvitation collections', {
              loeId: loe._id,
              clientId: loe.clientId
            });
        ...
        logger.error('❌ [LOE] Error fetching client data', {
          loeId: loe._id,
          clientId: loe.clientId,
          error: error.message
        });
      ...
      logger.error('❌ [LOE] No clientId found in LOE', {
        loeId: loe._id
      });
  ...
  try {
    ...
    if (loe.clientId) {
      try {
        ...
        logger.warn('⚠️ [LOE] Could not get client email for logging', { loeId: loe._id, error: error.message });
      }
    }
    ...
    if (loe.clientId) {
      try {
        ...
        if (client) {
          ...
          logger.info('✅ [LOE] Client data retrieved from Client collection for confirmation emails', {
            loeId: loe._id,
          ...
          if (invitation) {
            ...
            logger.info('✅ [LOE] Client data retrieved from ClientInvitation for confirmation emails', {
              loeId: loe._id,
              clientName: `${clientData.firstName} ${clientData.lastName}`,
              clientEmail: clientEmail
            });
          } else {
            logger.error('❌ [LOE] Client not found in either collection for confirmation emails', {
              loeId: loe._id,
              clientId: loe.clientId
            });
        ...
        logger.error('❌ [LOE] Error fetching client data for confirmation emails', {
          loeId: loe._id,
          clientId: loe.clientId,
          error: error.message
        });
        return { success: false, error: 'Failed to retrieve client information' };
      }
    } else {
      logger.error('❌ [LOE] No clientId found for confirmation emails', {
        loeId: loe._id
      });
      return { success: false, error: 'Client information not found' };
    }
