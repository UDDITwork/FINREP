/**
 * SIMPLE STOCK API TEST SCRIPT (ES Module Compatible)
 * 
 * Quick test to check which endpoints are working
 */
...
const testEndpoint = async (endpoint, params = {}) => {
  try {
    ...
    if (response.ok && data.success) {
      ...
      return { status: 'PASS', data: data.data };
    } else {
      log(`❌ ${endpoint} - FAILED (${response.status})`, 'red');
      return { status: 'FAIL', error: data.message };
    }
  } catch (error) {
